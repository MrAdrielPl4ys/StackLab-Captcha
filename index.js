...
const config = require("./config.json") // Puxando o conteúdo dentro do arquivo config.json
const fs = require("fs") // Definindo o FS
const client = new Discord.Client(); // Setando o nosso client, como 'client'
...
client.on('ready', () => { // setando o evento com nosso Discord.Client
    console.log(`Bot foi iniciado com sucesso`); // caso não haja erro, o bot enviará no console que ligou
    client.user.setActivity('Adicionando cargo aos usuários.')
});  

client.on('guildMemberAdd', membro => { // definimos o nome desse evento, como: membro
    var cargo = membro.guild.roles.cache.get("691904146357616660"); // puxamos o ID de um cargo, no qual, iremos adicionar para o membro
    ...
    canal.send(`${membro}`, embed).then(msg => { // criando um nome para a function 'then', no caso: msg
        ...
        let coletor = msg.createReactionCollector(filtro, {max: 1}); // criamos um coletor, para coletar informações do membro que reagiu
...
client.on('message', message => { // nome desse evento, foi setado como: message
    ...
    let command = client.commands.cache.get(cmd) // puxaremos o conteudo de tal comando
...
client.login(config.token); // Puxando o TOKEN definido no config.json!
